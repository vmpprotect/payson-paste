...
bool ImGui::CustomCheckbox(const char* label, bool* v)
{
    ...
    const float outline_scale = *v ? 1.0f : 0.5f;
    const ImU32 outline_col = ImGui::GetColorU32(*v ? ImVec4(0.0f, 1.0f, 0.2f, 1.0f) : ImVec4(0.0f, 0.8f, 0.15f, 0.5f));

    ImVec2 center = check_bb.GetCenter();
    center.x = IM_ROUND(center.x);
    center.y = IM_ROUND(center.y);
    const float radius = (square_sz - 2 * pad) * 0.5f;

    const float outline_thickness = 2.0f;
    window->DrawList->AddCircle(center, radius, outline_col, 0, outline_thickness);

    if (*v)
    {
        const float fill_radius = radius - outline_thickness;
        window->DrawList->AddCircleFilled(center, fill_radius, ImGui::GetColorU32(ImVec4(0.0f, 1.0f, 0.2f, 0.5f)));
    }

    static float t = 0.0f;
    t += ImGui::GetIO().DeltaTime * 2.0f;
    float glow_strength = 0.5f + 0.5f * sinf(t);
    const float glow_radius = radius + 2.0f;
    window->DrawList->AddCircle(center, glow_radius, outline_col & 0x30FFFFFF, 0, 4.0f * glow_strength);
...
bool ImGui::SliderFloatNeon(const char* label, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    // Slider behavior
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(frame_bb, id, ImGuiDataType_Float, v, &v_min, &v_max, format, flags, &grab_bb);

    // Calculate the width of the filled portion
    const float fill_width = grab_bb.Max.x - frame_bb.Min.x;

    // Gradient colors
    const ImU32 color_start = ColorConvertFloat4ToU32(ImVec4(0.0f, 1.0f, 0.2f, 0.7f)); // Bright neon green
    const ImU32 color_end = ColorConvertFloat4ToU32(ImVec4(0.0f, 0.5f, 0.1f, 0.3f));   // Darker, more transparent green

    // Draw gradient
    window->DrawList->AddRectFilledMultiColor(
        frame_bb.Min,
        ImVec2(frame_bb.Min.x + fill_width, frame_bb.Max.y),
        color_start,
        color_end,
        color_end,
        color_start);
    ...
    {
        window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(ImGuiCol_SliderGrab), style.GrabRounding);
    }
...
bool ImGui::NeonTabs(const char* str_id, const char* tabs[], int tabs_count, int* current_tab)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    // Set fixed width for the tab bar
    const float tab_bar_width = 80.0f;
    const float tab_height = 50.0f;
    const float tab_rounding = 12.0f;
    const float indicator_thickness = 3.0f;

    ImVec2 tab_bar_size(tab_bar_width, tab_height * tabs_count);
    ImRect bb(window->DC.CursorPos, window->DC.CursorPos + tab_bar_size);

    ItemSize(tab_bar_size, style.FramePadding.y);
    if (!ItemAdd(bb, id))
        return false;

    // Tab bar background
    window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(ImGuiCol_FrameBg), tab_rounding);

    // Tabs
    for (int i = 0; i < tabs_count; i++)
    {
        ImRect tab_bb(
            ImVec2(bb.Min.x, bb.Min.y + i * tab_height),
            ImVec2(bb.Max.x, bb.Min.y + (i + 1) * tab_height)
        );

        bool tab_selected = (i == *current_tab);
        bool tab_hovered = IsMouseHoveringRect(tab_bb.Min, tab_bb.Max);

        if (tab_hovered && IsMouseClicked(0))
            *current_tab = i;

        // Tab content
        ImVec2 text_size = CalcTextSize(tabs[i]);
        ImVec2 text_pos = ImVec2(
            tab_bb.GetCenter().x - text_size.x * 0.5f,
            tab_bb.GetCenter().y - text_size.y * 0.5f
        );

        ImU32 text_col;
        if (tab_selected)
        {
            text_col = GetColorU32(ImVec4(0.0f, 1.0f, 0.2f, 1.0f)); // Bright neon green for selected tab

            // Draw indicator for selected tab
            ImVec2 indicator_start(tab_bb.Min.x, tab_bb.GetCenter().y - indicator_thickness * 0.5f);
            ImVec2 indicator_end(tab_bb.Min.x + indicator_thickness, tab_bb.GetCenter().y + indicator_thickness * 0.5f);
            window->DrawList->AddRectFilled(indicator_start, indicator_end, text_col, indicator_thickness * 0.5f);
        }
        else if (tab_hovered)
        {
            text_col = GetColorU32(ImVec4(0.0f, 0.8f, 0.2f, 0.8f)); // Slightly dimmer neon green for hovered tab
        }
        else
        {
        ...
        window->DrawList->AddText(text_pos, text_col, tabs[i]);

        // Subtle hover effect
        if (tab_hovered && !tab_selected)
        {
            window->DrawList->AddRect(tab_bb.Min, tab_bb.Max, GetColorU32(ImVec4(0.0f, 1.0f, 0.2f, 0.3f)), tab_rounding);
        }
    }
